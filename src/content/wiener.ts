...
export const wienerLesson: LessonRecord = {
  ...
  intro: [
    'Tema 4 asks one question in four costumes: given an observed process $x(n)$ and a desired process $d(n)$, what fixed linear filter makes $\\mathbf{h}^H\\mathbf{x}(n)$ resemble $d(n)$ best in mean square? Identification, equalization, prediction and interference cancellation only differ in which wire carries which signal.',
    'The object to own is the error surface $\\xi(\\mathbf{h}) = P_d - 2\\operatorname{Re}\\{\\mathbf{h}^H\\mathbf{r}_{xd}\\} + \\mathbf{h}^H\\mathbf{R}_x\\mathbf{h}$: a bowl over filter space whose shape is set by $\\mathbf{R}_x$ and whose bottom is $\\mathbf{h}_{opt} = \\mathbf{R}_x^{-1}\\mathbf{r}_{xd}$. Tema 5 will roll a ball down this exact bowl, so time spent feeling its curvature pays twice.',
  ],
  modules: [
    {
      id: 'W1',
      ...
      trap: '$\\mathbf{R}_x\\mathbf{h}_{opt} = \\mathbf{r}_{xd}$ is memorized as “the formula,” and the orthogonality principle $E\\{\\mathbf{x}(n)e^*(n)\\} = \\mathbf{0}$ floats beside it as an unrelated slogan.',
      object:
        'A bowl. Every 2-tap filter $\\mathbf{h}$ is a point on the floor; the height above it is the honestly computed error power $\\xi(\\mathbf{h})$. The bowl is elliptical — its axes are the eigenvectors of $\\mathbf{R}_x$, its steepness along each axis is the eigenvalue. Somewhere on this floor is a lowest point.',
      action:
        'Drag $\\mathbf{h}$ around and watch two meters: the error power $\\xi(\\mathbf{h})$, and the correlation between the input and the residual error, $E\\{\\mathbf{x}e^*\\}$. Away from the bottom, the residual still correlates with the input — there is leftover predictable material the filter failed to use. Slide downhill and watch that correlation drain to zero exactly at the bottom. That is the orthogonality principle: the optimum is the point where the error has nothing left in common with the data.',
      payoff:
        'The normal equations are just “$\\nabla\\xi = 0$ at the bottom”: $\\mathbf{R}_x\\mathbf{h} = \\mathbf{r}_{xd}$ and $E\\{\\mathbf{x}e^*\\} = \\mathbf{0}$ are the same sentence in two notations. The minimum error $\\xi_{\\min} = P_d - \\mathbf{h}_{opt}^H\\mathbf{r}_{xd}$, and the penalty for being wrong by $\\Delta\\mathbf{h}$ is $\\Delta\\xi = \\Delta\\mathbf{h}^H\\mathbf{R}_x\\Delta\\mathbf{h}$ — the bowl’s own quadratic. When $\\mathbf{r}_{xd} = \\mathbf{0}$, the bottom is at $\\mathbf{h} = \\mathbf{0}$: a filter cannot manufacture resemblance where no correlation exists.',
      equations: [
        {
          sentence:
        ...
        },
        {
          ...
          caption:
            'And off the optimum: $\\xi = \\xi_{\\min} + \\Delta\\mathbf{h}^H\\mathbf{R}_x\\Delta\\mathbf{h}$ — the excess is the bowl itself.',
        },
      ],
      prediction: {
        question:
        ...
        answer:
          'At $\\mathbf{h} = \\mathbf{0}$, with depth $\\xi_{\\min} = P_d$. The best the filter can do is stay silent; filtering cannot create correlation.',
      },
      transfer:
        'System identification (exam staple): $\\mathbf{x} = \\mathbf{s} + \\mathbf{w}_x$, $d = \\mathbf{c}^H\\mathbf{s} + w_d$ gives $\\mathbf{h}_{opt} = \\frac{SNR}{SNR+1}\\,\\mathbf{c}$. Locate that answer on the bowl: noise on the input shrinks the bottom toward zero, never rotates it.',
      examRefs: [
        'Teoría T4 Ejemplo 1: identification, $\\mathbf{h}_{opt} = \\frac{SNR}{SNR+1}\\mathbf{c}$',
    ...
    },
    {
      ...
      object:
        'A concert hall. Microphone 1 hears music plus jackhammer: $x = s + w$. Microphone 2, placed near the window, hears mostly jackhammer with a music leak $\\alpha$: $y = \\alpha s + w$. The knob $h$ decides how much of mic 2 to subtract. Turn it too little and jackhammer survives; too much and you start subtracting the music’s leaked copy.',
      action:
        'Turn the knob and watch the residual power $P_z = (1-h\\alpha)^2 + (1-h)^2$ trace a parabola — the 1-D slice of the bowl. Find the bottom by hand; check it against $h_{opt} = \\frac{\\alpha+1}{\\alpha^2+1}$. Then move the microphone: slide $\\alpha$ toward 0 and watch cancellation become perfect ($h_{opt} = 1$, all jackhammer gone, music untouched); slide $\\alpha$ toward 1 and watch the two mics become the same signal — subtracting one from the other removes music and noise alike.',
      payoff:
        'The design rule falls out physically: the reference sensor must hear the interference, not the signal. $SNR_z = \\frac{(1-h_{opt}\\alpha)^2}{(1-h_{opt})^2}$ explodes as $\\alpha \\to 0$. This tiny system is also the perfect seed for Tema 5: replace the knob-turning hand with a rule that senses the slope of the parabola, and you have invented LMS.',
      equations: [
        {
          sentence:
          ...
          caption:
            'Unit-power music and noise, uncorrelated. $\\alpha = 0$ gives $h_{opt} = 1$ and total cancellation.',
        },
        {
          sentence:
          ...
          caption:
            '$\\rho$ is the correlation coefficient: the fraction of $d$’s power a linear filter can ever remove.',
        },
      ],
      prediction: {
        question: 'With $\\alpha = 1/2$, can the jackhammer be fully removed?',
        answer:
          'No. $h_{opt} = 6/5$ leaves both a music dent and residual noise: once the reference is contaminated, cancellation and self-erasure trade against each other.',
      },
      transfer:
    ...
    },
    {
      ...
      action:
        'Slide the tone’s SNR and watch the honestly computed optimum ($\\mathbf{h}_{opt} = \\mathbf{R}_x^{-1}\\mathbf{r}_x$ with $\\mathbf{R}_x = |A|^2\\mathbf{s}\\mathbf{s}^H + \\sigma^2\\mathbf{I}$) split the signal: prediction output captures $\\frac{Q \\cdot SNR}{1 + Q \\cdot SNR}$ of the tone; the error keeps the noise. Then look at the prediction-error filter’s frequency response: as SNR grows, a notch digs itself exactly at $f_0$ — no one told the filter the frequency; the autocorrelation did.',
      payoff:
        'Three exam objects become one: the ALE (delay long enough to decorrelate the wideband part, keep the narrowband correlation alive), the whitening interpretation (a long enough predictor eats all memory, leaving white error — used in DPCM and speech coding), and the eigen-story (as $\\sigma^2 \\to 0$, $\\mathbf{R}_x$ becomes rank-deficient, $\\xi_{\\min} \\to 0$, and the error filter places a zero on the unit circle at $e^{j2\\pi f_0}$). Prediction is Tema 1’s autocorrelation, weaponized.',
      equations: [
        {
          sentence:
        ...
        },
        {
          ...
          caption:
            '$SNR \\to 0$: filter gives up ($\\mathbf{h} \\to \\mathbf{0}$). $SNR \\to \\infty$: perfect prediction of the tone, $\\xi_{\\min} \\to \\sigma_w^2$, and the error filter has a spectral null at $f_0$.',
        },
      ],
      prediction: {
        question:
          '$M$ distinct tones in noise. What is the minimum predictor length that can cancel them all as $\\sigma^2 \\to 0$?',
        answer:
          '$Q = M$: the error filter needs $M$ zeros on the unit circle, one per tone — equivalently $\\mathbf{R}_t$ has rank $M$.',
      },
      transfer:
      ...
      examRefs: [
        ...
        'Final Jun 2025, Ex. 3: echo cancellation with regularized $\\mathbf{R}_x$ (adds $\\alpha\\mathbf{I}$ — shifts every eigenvalue)',
      ],
    },
  ],
  workedProblems: [
